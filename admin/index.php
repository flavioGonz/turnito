<?php
// /turnero/admin/index.php — Admin dark con pestañas y selector de animación (incluye efectos nuevos)
@session_start();
define('ADMIN_PASSWORD_DEFAULT', 'flavio20');
if (!function_exists('admin_check')) {
  function admin_check() {
    if (!isset($_SESSION['is_admin'])) {
      if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['password'])) {
        $pwd = trim($_POST['password']); $expect = ADMIN_PASSWORD_DEFAULT;
        if (is_file(__DIR__.'/../includes/config.php')) { include __DIR__.'/../includes/config.php'; if (defined('ADMIN_PASSWORD')) $expect = ADMIN_PASSWORD; }
      ...
      button{background:#3b82f6;border:0;margin-top:10px}</style>
      <form method="post"><h3 style="margin:0 0 10px">Admin</h3>
      <input type="password" name="password" placeholder="Contraseña" autofocus required>
      <button>Entrar</button></form>'; exit;
    }
  }
}
admin_check();
...
function list_media($dir){
  ...
  foreach(array_diff(scandir($dir),['.','..']) as $f){
    ...
    $out[]=['name'=>$f,'type'=>$type,'path'=>$p,'mtime'=>filemtime($p),'size'=>filesize($p)];
  }
...
$logos=[]; if(is_dir($logosDir)){ foreach(array_diff(scandir($logosDir),['.','..']) as $f) if(is_file($logosDir.'/'.$f)) $logos[]=$f; sort($logos,SORT_NATURAL|SORT_FLAG_CASE); }
$brandCfg=['global_logo'=>null,'per_queue'=>[]];
if(is_file($brandCfgFile)){ $tmp=json_decode(file_get_contents($brandCfgFile),true); if(is_array($tmp)) $brandCfg=array_merge($brandCfg,$tmp); }

$animMap=[]; if(is_file($animCfgFile)){ $tmp=json_decode(file_get_contents($animCfgFile),true); if(is_array($tmp)) $animMap=$tmp; }

/* Opciones de animación (incluye nuevas) */
$animOptions = [
  'none'            => '— Sin animación —',
  'kenburns'        => 'Ken Burns (rápida)',
  'pan_left'        => 'Paneo Izq→Der',
  'pan_right'       => 'Paneo Der→Izq',
  'pan_diagonal'    => 'Paneo diagonal',
  'zoom_in'         => 'Zoom In',
  'zoom_out'        => 'Zoom Out',
  'zoom_rotate'     => 'Zoom + Rotación',
  'tilt'            => 'Tilt/Rotate',
  'pulse'           => 'Pulso',
  'slide_up'        => 'Slide Up',
  'slide_left'      => 'Slide Left',
  'flash'           => 'Flash',
  // Nuevas (Minimamente-like)
  'puffin'          => 'Puffin (viva)',
  'vanishin'        => 'VanishIn (viva)',
  'swashing'        => 'Swashing (viva)',
  'foolishing'      => 'Foolishing (viva)',
  'tunupin'         => 'TunUpIn (viva)',
  'tindownin'       => 'TinDownIn (viva)',
  // Aleatorias
  'random_soft'     => 'Aleatoria (suaves)',
  'random_impact'   => 'Aleatoria (impacto/vivas)',
];

/* Colas (para pestaña Logos) */
$queues=[]; try{ require_once __DIR__.'/../db.php'; $st=$pdo->query("SELECT id,prefix,pad FROM queues ORDER BY id ASC"); $queues=$st->fetchAll(PDO::FETCH_ASSOC)?:[]; }catch(Throwable $e){}

$scriptDir=rtrim(dirname($_SERVER['SCRIPT_NAME']),'/'); $basePath=preg_replace('#/admin$#','',$scriptDir); if($basePath==='/') $basePath='';
?>
<!doctype html>
<html lang="es">
<head>
  ...
  <style>
    ...
    .sel{background:#0b0f14;border:1px solid #1f2937;color:#e5e7eb;border-radius:10px;padding:6px 8px}
    code{color:#9ae6b4}
    .cap{font-size:.8rem;color:#93a4b8}
  </style>
</head>
<body class="p-3">
        ...
        <div class="row g-3">
          <div class="col-lg-4">
            <div class="card p-3 h-100">
              ...
              <hr>
              <div class="cap">Asigná una <b>animación</b> distinta a cada imagen. Los videos no usan animación.</div>
            </div>
          </div>
          <div class="col-lg-8">
            <div class="card p-3">
              ...
              <div class="d-flex flex-wrap gap-3">
                ...
                <?php foreach ($activeList as $it):
                  ...
                  $selAnim = $animMap[$it['name']] ?? 'kenburns';
                ?>
                <div style="width:210px">
                  ...
                  <div class="mt-1 small text-muted text-truncate" style="max-width:200px" title="<?=h($it['name'])?>"><?=h($it['name'])?></div>

                  <?php if ($it['type']==='image'): ?>
                  <form class="mt-2" method="post" action="animation_save.php">
                    <input type="hidden" name="file" value="<?=h($it['name'])?>">
                    <div class="d-flex gap-2">
                      <select name="anim" class="sel" style="flex:1">
                        <?php foreach($animOptions as $key=>$label): ?>
                          <option value="<?=h($key)?>" <?=$selAnim===$key?'selected':''?>><?=h($label)?></option>
                        <?php endforeach; ?>
                      </select>
                      <button class="btn btn-sm btn-soft">Aplicar</button>
                    </div>
                  </form>
                  <?php else: ?>
                    <div class="cap mt-2">Video (sin animación)</div>
                  <?php endif; ?>
        ...
        <div class="row g-4">
          ...
          <div class="col-lg-7">
            <div class="card p-3">
              ...
              <?php $brandSel = $brandCfg['global_logo'] ?? ''; ?>
              <form method="post" action="logo_save.php">
                <div class="mb-3">
                  ...
                  <select name="global_logo" class="form-select">
                    ...
                    <?php foreach ($logos as $f): ?>
                      <option value="<?=h($f)?>" <?=$brandSel===$f?'selected':''?>><?=h($f)?></option>
                    <?php endforeach; ?>
                  </select>
                ...
                <?php if ($queues): ?>
                  <hr><h6 class="mb-2">Logo por cola (opcional)</h6>
